...
def run_epoch(model, dataloader, epoch, optimizer=None, scheduler=None):
    ...
    for x, y in loader_bar:
        ...
        if optimizer:
            loader_bar.set_description("Train epoch {}, loss: {:.4f}"
...
def finetune(args: DictConfig) -> None:
    ...
    imagenet_dir = os.path.join(result_dir, 'ImageNet_train')
    os.makedirs(imagenet_dir, exist_ok=True)

    log_dir = os.path.join(imagenet_dir, 'log')
    os.makedirs(log_dir, exist_ok=True)

    model_dir = os.path.join(imagenet_dir, 'models')
    ...
    for epoch in range(1, args.finetune_epochs + 1):
        ...
        if train_loss < optimal_loss:
            ...
            save_dir = os.path.join(model_dir, 'imagenet_best.pth')
