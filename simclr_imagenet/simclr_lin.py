...
def finetune(args: DictConfig) -> None:
    ...
    Linear_dir = os.path.join(result_dir, 'Linear_Classification_Protocol')
    os.makedirs(Linear_dir, exist_ok=True)

    log_dir = os.path.join(Linear_dir, 'log')
    os.makedirs(log_dir, exist_ok=True)

    model_dir = os.path.join(Linear_dir, 'models')
    ...
    load_model_dir = os.path.join(train_dir, 'models')
    os.makedirs(load_model_dir, exist_ok=True)



    train_set = CIFAR100(root=data_dir, train=True, transform=train_transform, download=True)
    ...
    load_model_dir1 = os.path.join(load_model_dir, 'simclr_{}_epoch{}.pt'.format(args.backbone, args.load_epoch))

    pre_model.load_state_dict(torch.load(load_model_dir1))
    ...
    for epoch in range(1, args.finetune_epochs + 1):
        ...
        if train_loss < optimal_loss:
            ...
            save_dir = os.path.join(model_dir, 'simclr_lin_{}_best.pth'.format(args.backbone))
