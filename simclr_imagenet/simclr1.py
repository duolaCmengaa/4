...
def train(args: DictConfig) -> None:
    ...
    test_set = CIFAR10Pair(root=data_dir,
                            train=False,
                            transform=train_transform,
                            download=True)    

    # 将 train_set 和 test_set 合并为一个数据集
    merged_dataset = ConcatDataset([train_set, test_set])
    print("dataset length",len(merged_dataset))

    train_loader = DataLoader(merged_dataset,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 记录损失到 TensorBoard
        writer.add_scalar('Loss/train', loss_meter.avg, epoch * len(train_bar))
        ...
        if epoch >= args.log_interval and epoch % (args.log_interval*10) == 0:
            save_dir = os.path.join(model_dir, 'simclr_{}_epoch{}.pt'.format(args.backbone, epoch))
